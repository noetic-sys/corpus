        ...
        try:
            ...
            chunk_models = [
                ChunkIndexingModel(
                    ...
                    company_id=company_id,  # Use parameter, not chunk attribute
                    content=c.content,
                    metadata=c.metadata,
                )
                for c in chunks
            ...
            if success:
                ...
                activity.logger.error(
                    f"Failed to index chunks for document {document_id}"
                )
                return IndexChunksResult(
                    success=False,
                    indexed_count=0,
            ...
            activity.logger.error(
                f"Error indexing chunks for document {document_id}: {e}"
            )
            return IndexChunksResult(
                success=False,
                indexed_count=0,
