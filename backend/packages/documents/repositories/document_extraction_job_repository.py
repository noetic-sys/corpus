...
class DocumentExtractionJobRepository(
    ...
    def __init__(self, db_session: AsyncSession):
        super().__init__(
            DocumentExtractionJobEntity, DocumentExtractionJobModel, db_session
        )
    ...
    async def get_pending_documents(self) -> List[DocumentModel]:
        ...
        result = await self.db_session.execute(
            select(DocumentEntity).where(
                and_(
                    DocumentEntity.extraction_status == ExtractionStatus.PENDING.value,
    ...
    async def get_failed_jobs(
        ...
        result = await self.db_session.execute(
            select(self.entity_class)
            .where(self.entity_class.status == DocumentExtractionJobStatus.FAILED.value)
            .order_by(self.entity_class.id.desc())
            .limit(limit)
        )
        entities = result.scalars().all()
