...
class DocumentRepository(BaseRepository[DocumentEntity, DocumentModel]):
    def __init__(self):
        super().__init__(DocumentEntity, DocumentModel)
    ...
    async def get_valid_ids(
        ...
        async with self._get_session() as session:
            ...
            if company_id:
                valid_docs_query = self._add_company_filter(
                    valid_docs_query, company_id
                )
            result = await session.execute(valid_docs_query)
