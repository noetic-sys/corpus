...
class AIResponseParser:
    ...
    def parse_response(
        ...
        try:
            ...
            logger.info(f"RAW AI RESPONSE (len={len(json_response) if json_response else 0}): {repr(json_response)}")
            ...
            logger.info(f"After cleaning (len={len(json_response)}): {repr(json_response)}")
