...
async def launch_agent_qa_activity(
    ...
    """
    activity.logger.info(f"Launching agent QA for job {qa_job_id}")
...
async def check_agent_qa_status_activity(
    ...
    """
    return await check_execution_status(execution_info)
...
async def cleanup_agent_qa_activity(
    ...
    await cleanup_execution_resources(
        execution_info, company_id, cleanup_execution_service_account
    )
