...
from packages.workflows.workers.workflow_temporal_worker import WorkflowTemporalWorker


def setup_cli():
    """Setup CLI arguments and return parsed args with factory parameters."""
    parser = argparse.ArgumentParser(description="Temporal Workflow Execution Worker")
    parser.add_argument(
        "--temporal-host",
        type=str,
...
def main():
    ...
    WorkerLauncher().run_with_cli(
        worker_factory=WorkflowTemporalWorker,
        worker_name="Workflow Execution Worker",
        setup_models=True,
